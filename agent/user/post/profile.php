...
if (isset($_POST['edit_your_user_password'])) {
    ...
    // v2 wrapping invalidated; regenerated by unlockUserMasterKey() on next login.
    // Phase 10: keypair invalidated too — privkey was wrapped under the old
    // password's KEK. Drop pubkey/privkey and grants; backfillUserCryptoMaterial
    // will regenerate them at next login from user_client_permissions.
    mysqli_query($mysqli,"UPDATE users SET user_password = '$new_password', user_specific_encryption_ciphertext = '$user_specific_encryption_ciphertext', user_specific_encryption_ciphertext_v2 = NULL, user_pubkey = NULL, user_privkey_wrapped = NULL WHERE user_id = $session_user_id");
    mysqli_query($mysqli, "DELETE FROM user_client_grants WHERE user_id = $session_user_id");
